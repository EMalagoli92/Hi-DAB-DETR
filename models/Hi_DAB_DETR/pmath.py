"""
Implementation of mathematical operations in the Poincaré ball model.
Adapted from:
...
def _mobius_matvec(m, x, c):
    ...
    mx_norm = torch.clamp_min(mx.norm(dim=-1, keepdim=True, p=2), 1e-5)
...
def mobius_scalar_mul(r, x, *, c=1.0):
    ...
    r \otimes_c x = \frac{1}{\sqrt{c}} \tanh\left(
    r \cdot \tanh^{-1}(\sqrt{c} \|x\|) \right)
    \cdot \frac{x}{\|x\|}
    """
    c = torch.as_tensor(c).type_as(x)
...
def _mobius_scalar_mul(r, x, c):
    sqrt_c = c ** 0.5
    x_norm = torch.clamp_min(x.norm(dim=-1, keepdim=True, p=2), 1e-5)
    res = tanh(r * artanh(sqrt_c * x_norm)) * x / (sqrt_c * x_norm)
    return _project(res, c)
...
def geodesic(
    ...
    *,
    ...
    """
    Compute the point on the Möbius geodesic between x and y.

    This corresponds to:
        gamma(t) = x +_c (-x +_c y) x_c t

    The path can also be treated as an extension of the line segment to an
    unbounded geodesic that goes through x and y.
    """
    diff = mobius_add(-x, y, c=c)
    scaled = mobius_scalar_mul(t, diff, c=c)
    return mobius_add(x, scaled, c=c)
